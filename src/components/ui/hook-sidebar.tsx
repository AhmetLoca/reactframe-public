...
export type HookSidebarItem = string | { label: string; href?: string; count?: number };
...
const countOf = (item: HookSidebarItem) => (typeof item === "string" ? undefined : item.count);
...
export function HookSidebar({
  ...
  return (
    <nav data-slot="hook-sidebar" aria-label={label} className={cn("flex flex-col", className)} {...props}>
      {label && (
        <span data-slot="hook-sidebar-label" className="pb-3 pl-0.5 pr-2 font-sans text-sm font-medium uppercase tracking-wide text-foreground">
          {label}
        </span>
      )}

      <div ref={listRef} onMouseLeave={() => setPointerInside(false)} className="relative flex flex-col gap-0.5">
        <Rail from={hoverFrom} y={hoverY} visible={(pointerInside || focusInside) && hoverIndex !== activeIndex} dashed={dashed} className="text-foreground/30" />
        ...
        {items.map((item, index) => {
          ...
          const rowProps = {
            ...
            className: cn(
              "flex items-center justify-between gap-2 rounded-lg py-1.5 pl-5 pr-2 text-left text-sm transition-colors duration-200 motion-reduce:transition-none",
          ...
          const content = (
            <>
              {text}
              {count !== undefined && <span className={cn("text-xs", isActive ? "text-foreground/45" : "text-foreground/30")}>{count}</span>}
            </>
          );

          return href ? (
            <Link key={`${index}-${text}`} {...rowProps} ref={setRef as React.Ref<HTMLAnchorElement>} href={href} aria-current={isActive ? "page" : undefined}>
              {content}
            </Link>
          ) : (
            <button key={`${index}-${text}`} {...rowProps} ref={setRef as React.Ref<HTMLButtonElement>} type="button" aria-current={isActive ? "true" : undefined}>
              {content}
            </button>
          );
        })}
